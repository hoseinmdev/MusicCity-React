...
const TrackBox: React.FC<ITrack> = ({
  ...
  return (
    <Link
      ...
      className={`group/trackBox relative flex w-32  flex-col items-start justify-between gap-2 text-sm dark:text-white lg:w-auto lg:lg:cursor-pointer lg:text-base`}
    >
      <img
        className={`h-full w-full rounded-lg hover:brightness-75 dark:hover:opacity-70 ${
      ...
      <button
        className={`absolute bottom-[-5px]  right-1 hidden text-6xl text-white opacity-0 hover:scale-105 group-hover/trackBox:opacity-100 lg:block lg:group-hover/trackBox:bottom-[4.25rem] ${
